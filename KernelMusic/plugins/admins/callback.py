...
from KernelMusic import YouTube, app
from KernelMusic.core.call import Kernel
from KernelMusic.misc import SUDOERS, db
from KernelMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from KernelMusic.utils.decorators.language import languageCB
from KernelMusic.utils.formatters import seconds_to_min
from KernelMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from KernelMusic.utils.stream.autoclear import auto_clean
from KernelMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Kernel.pause_stream(chat_id)
        ...
        await Kernel.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Kernel.stop_stream(chat_id)
        ...
        await Kernel.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Kernel.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Kernel.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Kernel.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Kernel.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
